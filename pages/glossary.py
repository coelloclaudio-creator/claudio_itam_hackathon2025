...
The idea is to use this information to sign or scout players who currently play at teams belonging to clusters that transition well to América’s cluster. This goes one step further than a simple “this player has performed well, let’s sign him and see how he does in our scheme”. The clustering is an attempt to adjust to schematic differences and team-level context. 
...
Statsbomb does not partition Liga MX stats into apertura and clausura, which leaves a lot of holes and unknowns in this analysis. If we had access to an accurate database, there would be a lot more transitions counted and more granular data to work with.

A database including 10 or 15 years of league play would greatly increase the scope of the analysis, and we could even see how teams have changed their playing styles over the years.

Access to more advanced scouting data, such as physical combine results, projected transfer fees, and even previous data from players that have played in other competitions would help better predict how a player might perform at América. 

This is meant to be a tool to enhance scouting decisions, but a responsible scouting department would not use it as the end-all-be-all. A team needs to cross check the clustering data with their own “eye test” to see if the players our matrix identifies as good fits would actually meet the team’s current needs.
"""
